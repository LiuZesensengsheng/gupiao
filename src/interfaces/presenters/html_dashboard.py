  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .hero {{
      ...
      overflow: hidden;
      background:
        linear-gradient(135deg, rgba(255, 252, 246, 0.96), rgba(248, 239, 224, 0.96)),
        var(--panel);
      border: 1px solid var(--line);
      border-radius: 28px;
      padding: 26px;
    ...
    .hero::after {{
      ...
      height: 220px;
      border-radius: 50%;
      background: radial-gradient(circle, rgba(31, 110, 120, 0.12), transparent 70%);
    }}
    .hero-top {{
      display: flex;
    ...
    .hero-card {{
      ...
      border: 1px solid rgba(201, 185, 149, 0.55);
      border-radius: 20px;
      padding: 16px;
    ...
    .panel {{
      ...
      border-radius: 24px;
      padding: 18px;
      box-shadow: 0 12px 28px rgba(45, 31, 8, 0.05);
    }}
    .panel-title {{
      display: flex;
      align-items: center;
    ...
    .budget-fill {{
      height: 100%;
      border-radius: 999px;
      background: linear-gradient(90deg, var(--accent), #d98b4f);
    }}
    .list {{
      margin: 0;
      padding-left: 18px;
  ...
  <main class="page">
    <section class="hero">
      ...
      <div class="hero-strip">
        <article class="hero-card">
          <div class="eyebrow">Target Exposure</div>
          <div class="metric">{_pct(policy.target_exposure)}</div>
          <div class="metric-sub">Target positions {policy.target_position_count} | turnover cap {_pct(policy.turnover_cap)}</div>
        </article>
        <article class="hero-card">
          <div class="eyebrow">Market Stack</div>
          <div class="metric">{_pct(market.up_20d_prob)}</div>
          <div class="metric-sub">1d {_pct(market.up_1d_prob)} | 5d {_pct(market.up_5d_prob)} | 20d {_pct(market.up_20d_prob)}</div>
        </article>
        <article class="hero-card">
          <div class="eyebrow">Execution State</div>
          <div class="metric">{buy_count + sell_count}</div>
          <div class="metric-sub">Buy {buy_count} | Sell {sell_count} | intraday T {"on" if policy.intraday_t_allowed else "off"}</div>
        </article>
        <article class="hero-card">
          <div class="eyebrow">External Layer</div>
          <div class="metric">{escape(capital.flow_regime)}</div>
          <div class="metric-sub">Macro {escape(macro.macro_risk_level)} | info items {result.info_item_count}</div>
        </article>
      </div>
    </section>
    ...
    <section class="section section-grid">
      <article class="panel">
        <div class="panel-title">
          <div>
            <h2>Execution Spotlight</h2>
            <p>Only the actionable plan and the highest-signal reasons stay above the fold.</p>
          </div>
          {_badge(f"{len(top_actions)} shown", "neutral")}
        </div>
        ...
        <div class="callout">
          <strong>Decision Drivers</strong>
          <ul class="list">{risk_notes_html}</ul>
        </div>
      </article>
      ...
      <article class="panel">
        <div class="panel-title">
          <div>
            <h2>Market Pulse</h2>
            <p>Compact gauges for tomorrow's decision context.</p>
          </div>
        </div>
    ...
    <section class="section section-grid">
      <article class="panel">
        <div class="panel-title">
          <div>
            ...
            <p>What the portfolio actually wants to hold after all risk clamps.</p>
          </div>
          {_badge(f"{len(selected_stocks)} selected", "neutral")}
        </div>
        ...
        <div class="callout">
          <strong>Sector Budget</strong>
          <div class="budget-stack">{sector_budget_rows or "<div class='empty'>No sector budget assigned.</div>"}</div>
        </div>
      </article>
      ...
      <article class="panel">
        <div class="panel-title">
          <div>
            <h2>Watchlist Edge</h2>
            <p>Top-ranked candidates only. The full 300-name table is collapsed below.</p>
          </div>
          {_badge(f"top {len(watchlist_stocks)}", "neutral")}
        </div>
        <div class="table-wrap">
          <table>
            <thead>
              <tr><th>Rank</th><th>Name</th><th>1d</th><th>5d</th><th>20d</th><th>Alpha</th><th>Tradeability</th><th>Target</th></tr>
            </thead>
            <tbody>
              {watch_rows_html or "<tr><td colspan='8'>No candidates.</td></tr>"}
            </tbody>
          </table>
        </div>
      </article>
    ...
    <section class="section triple-grid">
      <article class="panel">
        <div class="panel-title">
          <div>
            ...
            <p>Capital flow and macro context in one glance.</p>
          </div>
          {_badge("enabled" if result.external_signal_enabled else "disabled", "neutral")}
        </div>
        <div class="mini-grid">
      ...
      <article class="panel">
        <div class="panel-title">
          <div>
            <h2>Info Overlay</h2>
            <p>This section should only matter when the overlay is non-empty.</p>
          </div>
          {_badge("on" if result.info_shadow_enabled else "off", "neutral")}
        </div>
        <div class="mini-grid">
      ...
      <article class="panel">
        <div class="panel-title">
          <div>
            ...
            <p>Recent recurring symbols, risk tags, and narrative.</p>
          </div>
          {_badge(f"{result.memory_recall.recent_daily_run_count} runs", "neutral")}
        </div>
        <div class="mini-grid">
    ...
    <section class="section section-grid">
      <article class="panel">
        <div class="panel-title">
          <div>
            <h2>Negative Event Watch</h2>
            <p>Only top negatives are worth surfacing here.</p>
          </div>
        </div>
      ...
      <article class="panel">
        <div class="panel-title">
          <div>
            <h2>Positive Signal Watch</h2>
            <p>Positive signals should support selection, not dominate it.</p>
          </div>
        </div>
    ...
    <section class="section">
      <article class="panel">
        <div class="panel-title">
          <div>
            <h2>Quant vs Info Divergence</h2>
            <p>Only the largest disagreements are shown.</p>
          </div>
        </div>
    ...
    <section class="section">
      <details>
        ...
        <div class="details-body">
          <div class="table-wrap">
            <table>
              <thead>
                <tr><th>Rank</th><th>Name</th><th>1d</th><th>2d</th><th>3d</th><th>5d</th><th>20d</th><th>Alpha</th><th>Excess vs sector</th><th>Tradeability</th><th>Target</th></tr>
              </thead>
              <tbody>
                {full_rows_html}
              </tbody>
      ...
      <div class="footer-note">
        This dashboard is designed as a decision page first and an audit page second. The actionable plan stays above the fold; the full universe remains available below for review.
      </div>
    </section>
  </main>
</body>
