...
def build_stock_live_panel_dataset(
    *,
    stock_securities: Sequence[Security],
    source: str,
    data_dir: str,
    ...
    for security in stock_securities:
        ...
        raw_dates = pd.to_datetime(stock_raw.get("date"), errors="coerce").dropna().sort_values()
        if not raw_dates.empty:
            raw_latest_dates[security.symbol] = pd.Timestamp(raw_dates.iloc[-1])
        symbol_status[security.symbol] = _infer_symbol_status(stock_raw=stock_raw)
        frame = make_stock_feature_frame(stock_raw, market_frame)
    ...
    for col in feature_cols:
        ...
        panel[col] = panel.groupby("symbol", observed=True)[col].ffill()
        if panel[col].isna().any():
            median = panel[col].median(skipna=True)
            panel[col] = panel[col].fillna(0.0 if pd.isna(median) else float(median))

    panel["tradability_status"] = panel["symbol"].map(symbol_status).fillna(STATUS_NORMAL)
    return StockPanelDataset(
        frame=panel,
        feature_columns=feature_cols,
        notes=notes,
