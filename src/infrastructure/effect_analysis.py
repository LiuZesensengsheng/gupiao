...
from src.domain.entities import EffectSummary, Security
        ...
        rows.append(
            {
                "symbol": symbol,
                "name": sec.name,
                "sector": sector_map.get(symbol, sec.sector),
...
def compute_sector_table(snapshot: pd.DataFrame, top_k: int = 8) -> pd.DataFrame:
    ...
    for sector, group in grouped:
        heat = _clip(
            0.35 * group["ret_5"].mean()
            + 0.25 * group["trend_5_20"].mean()
            + 0.20 * (group["vol_ratio_20"].mean() - 1.0)
            + 0.20 * (group["obv_z_20"].mean() / 2.0),
            -1.0,
            1.0,
        )
        money = _clip(0.7 * (group["vol_ratio_20"].mean() - 1.0) + 0.3 * (group["obv_z_20"].mean() / 2.0), -1.0, 1.0)
        chip = _clip(0.7 * ((group["price_pos_20"].mean() - 0.5) * 2) + 0.3 * (group["vol_conc_5_20"].mean() - 0.25) * 3, -1.0, 1.0)
        rows.append(
            {
                "sector": str(sector),
                "heat_score": float(heat),
                "win_rate_1d": float((group["ret_1"] > 0).mean()),
                "median_ret_5d": float(group["ret_5"].median()),
